...
def fft_core(ASD, ASD_noNA, D, max_proportion_NA, stepD_cM, qbins, bins_left_bound):
    ...
    INDEX = []
    for i in range(len(subbins)):
        x = np.where(pos_bin==i)[0]
        if len(x)==0:
            INDEX = INDEX + [-1]
        else:
            INDEX = INDEX + [x[0]]
...
with open(output_prefix+'.perjk.fits', 'w') as fout:
    fout.write('pop\tchrom\tA\tt\tc\tblockweights\n')
    for i in range(R.shape[0]-3):
        fout.write(target_popname+'\t'+str(i+1)+'\t'+'\t'.join([str(x) for x in R[i,0:3].A1])+'\t'+str(R[i,4])+'\n')
