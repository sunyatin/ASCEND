...
# 3 feb 2020: v8.1.1: speeded up the assignation of the SNPs to the subbins (@fft_core) + added possibility to specify multiple chr to analyze + user can provide None as option argument and this will be converted to np.None
# 4 feb 2020: v8.1.2: added sys.exit in case where chr provided does not exist in the snp file
# 8 feb 2020: v8.2: added the option "randomhet" to randomly pick up 0 or 2 shared alleles in case of hetero ind pairs (instead of setting 1, by default)
"""
...
def number_shared_alleles(pop1, pop2, randomhet = False):
    dif = 2 - abs(pop1 - pop2)
    prod = pop1 * pop2
    if randomhet == True:
...
def calculate_allele_sharing(input_prefix, 
    ...
    start = time.time()
    if out_popname != None:
        print2('Analyzing '+target_popname+' and using an outgroup population: '+out_popname)
    else:
        print2('Analyzing only '+target_popname+', without outgroup.')
    
    D_FULL = np.genfromtxt(input_prefix[1], dtype=float, usecols = (1,2))
    CHR = D_FULL[:,0].astype(int)
    D_FULL = D_FULL[:,1]
    if input_distance_in_cM == False:
        print2('Converting Morgans input into centiMorgans')
        D_FULL = 100.0 * D_FULL
    POP = np.genfromtxt(input_prefix[2], dtype=str, usecols = 2)
    
    if out_popname == None:
        print2('There are '+str(np.sum(POP==target_popname))+' target samples\n')
    else:
        print2('There are '+str(np.sum(POP==target_popname))+' target and '+str(np.sum(POP==out_popname))+' outgroup samples\n')
    ...
    print2('Chromosomes: '+' '.join([str(x) for x in chr_values])+'\n')
    ...
    for chrom in chr_values:
        ...
        nSNP = G.shape[0]
        print2('Raw nSNP: '+str(nSNP_raw)+'   MAF-filtered nSNP: '+str(nSNP))
        ...
        for j in range(0, G.shape[1]):
            for k in range(j+1, G.shape[1]):
                ASD1[:,i] = number_shared_alleles(G[:,j], G[:,k], randomhet)
                i += 1
        ...
        if out_popname != None:
            ...
            for j in range(0, G.shape[1]):
                for k in range(0, G_out.shape[1]):
                    ASD2[:,i] = number_shared_alleles(G[:,j], G_out[:,k], randomhet)
                    i += 1
        ...
        print2('There will be '+str(n_bins)+' bins.')
...
def expfit_1D_jackknife(input, blocksizes, output_prefix, min_D_cM = 0, max_D_cM = 30.0, use_substracted = False):
    ...
    print2('\tChromosomes in input file:\n'+' '.join([str(int(x)) for x in np.unique(F[:,0]).tolist()]))
    print2('\tChromosomes at the intersection of the two files:\n'+' '.join([str(x) for x in intersection]))
...
print2('\n\n                ᗩ ᔕ ᑕ Eᑎ ᗪ                \n\n')
print2('Version '+str(version)+'\n')
print2(genotypename+' '+snpname+' '+indivname)
print2('Output file name: '+output_prefix)
print2('Target population: '+target_popname)
print2('Out population: '+str(out_popname))
print2('minMAF: '+str(minMAF))
if input_geno_is_diploid:
    print2('Assumes the genotypes are diploids')
else:
    print2('Assumes the genotypes are haploids')
if pseudodiploidize:
    print2('Converting genotypes into pseudohomozygous')
print2('Distance bins: '+str(minD_cM)+' cM to '+str(maxD_cM)+' cM by steps of '+str(stepD_cM)+' cM')
print2('Input distance in cM: '+str(input_distance_in_cM))
print2('Maximum proportion of allele sharing values that can be missing: '+str(max_proportion_NA))
if blockSizeFile == None:
    print2('No block size file provided, will use the number of SNPs per chromosome as weights')
else:
    print2('Block size file: '+blockSizeFile)

if usefft:
    print2('\nUsing FFT algorithm')
    print2('qbins: '+str(qbins))
else:
    print2('\nUsing naive algorithm')
    
if randomhet:
    print2('CAUTION! You set "randomhet: YES" which means that in case of a pair of individuals being heterozygous at a SNP, the number of alleles shared will be randomly set as 0 or 2 (instead of 1).')
...
if onlyfit == False:
    ...
    print2('Estimating the parameters only, with the weighted block jackknife procedure.')
    print2('Input: '+output_prefix+'.out')

if out_popname == None:
    use_substracted = False
    print2('Not substracting by cross-population allele sharing correlation')
else:
    use_substracted = True
    print2('Substracting by cross-population allele sharing correlation')

# run the expfit with jackknife procedure
print2('============================================================================================')
print2('Running the exponential fitting with weighted jackknife')
